export type WhatiuseAgentRecipeId = "issues-workspace" | "customer-directory" | "audit-log" | "saas-overview" | "product-usage" | "conversion-retention" | "customer-workspace" | "billing-usage" | "members-permissions";
...
export declare const whatiuseAgentRecipeContracts: readonly [{
    ...
    readonly registryItem: "whatiuse-data";
    ...
    readonly registryItem: "whatiuse-analytics";
    ...
    readonly title: "Usage & Adoption";
    readonly domain: "analytics";
    readonly intent: "Filter accounts, compare usage and adoption, and follow activation into the supporting records.";
    readonly signals: readonly ["product usage", "feature adoption", "active usage", "activation path", "usage explorer", "account adoption"];
    readonly registryItem: "whatiuse-analytics";
    readonly modulePath: "components/patterns/analytics-recipes";
    ...
    readonly state: readonly ["local active index", "date range", "account filters", "selected feature", "selected stage", "selected account"];
    readonly rules: readonly ["Keep transient chart inspection local.", "Cross-filter only from explicit feature and stage selection.", "Keep account detail and its source row on one stable id."];
    readonly forbidden: readonly ["Do not mirror hover state into account filters.", "Do not hide exact account values behind the chart.", "Do not animate high-frequency inspection."];
}, {
    ...
    readonly registryItem: "whatiuse-product-patterns";
...
export declare const whatiuseAgentSelectionRules: readonly [{
    ...
    readonly task: "Inspect a numeric distribution";
    readonly choose: "Histogram";
    readonly insteadOf: readonly ["Chart", "Breakdown"];
    readonly when: readonly ["Continuous or bucketed values need shape, spread, and outlier inspection."];
    readonly rejectWhen: readonly ["The x-axis is an ordered time series.", "The values are unrelated categories."];
}, {
    readonly task: "Inspect a relationship between two measures";
    readonly choose: "ScatterChart";
    readonly insteadOf: readonly ["Chart", "Breakdown"];
    readonly when: readonly ["Both axes are continuous and correlation, clusters, or outliers matter."];
    readonly rejectWhen: readonly ["Only one measure exists.", "The x-axis is primarily time."];
}, {
    readonly task: "Explain a running value bridge";
    readonly choose: "WaterfallChart";
    readonly insteadOf: readonly ["BarChart", "Breakdown"];
    readonly when: readonly ["Ordered positive and negative contributions reconcile a known start and end."];
    readonly rejectWhen: readonly ["Bars are independent categories.", "The sequence does not reconcile to a meaningful total."];
}, {
    readonly task: "Compare a bounded multi-axis profile";
    readonly choose: "RadarChart";
    readonly insteadOf: readonly ["BarChart", "Breakdown"];
    readonly when: readonly ["A small set of normalized dimensions forms a meaningful profile."];
    readonly rejectWhen: readonly ["Exact cross-category ranking matters.", "Scales are not comparable or there are too many axes."];
}, {
    readonly task: "Show one bounded current measure";
    readonly choose: "Gauge";
    readonly insteadOf: readonly ["Goal", "Progress"];
    readonly when: readonly ["A single current value has an explicit minimum, maximum, and concise status meaning."];
    readonly rejectWhen: readonly ["Trend or history matters.", "The value is an indeterminate process."];
}, {
    readonly task: "Inspect a small directed flow";
    readonly choose: "SankeyChart";
    readonly insteadOf: readonly ["Funnel", "Flow diagram"];
    readonly when: readonly ["Named sources, destinations, and weighted transfers form a small acyclic network."];
    readonly rejectWhen: readonly ["The flow is a single ordered conversion path.", "The graph is cyclic, dense, or needs general node editing."];
}];
export declare const whatiuseAgentForbiddenRules: readonly ["Do not invent undocumented props, exports, registry items, or private CSS selectors.", "Do not merge components because their closed shapes look similar; preserve task, focus, and recovery boundaries.", "Do not add a primitive when an existing recipe already owns the product task.", "Do not persist selection, open overlays, pointer position, loading state, or in-flight work.", "Do not place column sizing, pinning, or visual tooltip state in server requests.", "Do not use a Toast when the user must respond or when status must remain visible.", "Do not use Alert Dialog when a reliable Undo path exists.", "Do not hide exact values behind color, charts, progress fills, or iconography.", "Do not remove semantic tables or text summaries from analytic visuals.", "Do not animate high-frequency inspection, keyboard navigation, sorting, filtering, or virtual scrolling.", "Do not move focus on pointer activation; reserve visible focus treatment for keyboard navigation.", "Do not change collection geometry when detail, selection, bulk actions, loading, or feedback appears.", "Do not create a new feedback stack for sequential outcomes; update one foreground feedback identity.", "Do not claim production adoption, external review, or device coverage without evidence."];
export declare const whatiuseAgentSystemContract: {
    readonly schemaVersion: 1;
    readonly product: "whatiuse";
...
export declare function selectWhatiuseRecipe(task: string): WhatiuseAgentPlan | null;
