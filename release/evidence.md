# Generated release evidence

> Generated by `scripts/build-evidence.mjs`. Do not edit this file directly.

| Contract | Verified source count |
| --- | ---: |
| Version | `0.1.0-rc.30` |
| Public components | 45 |
| Product primitives | 17 |
| Registry items | 74 |
| Public exports | 356 |
| Semantic tokens | 145 |
| Public routes | 69 |
| Source tests | 238 |
| Storybook contract runs | 104 |
| Release visual baselines per platform | 332 |
| Browser projects | 5 |
| Browser matrix | 353 passed, 352 skipped |
| Fresh quickstart consumer | passed in 15.8 seconds |
| Adoption DX | 4 automated journeys, longest 15.8 seconds |
| Package SSR and hydration | passed, 201 runtime exports, 0 recoverable errors |
| Fresh Teum Analytics consumer | passed in 10.9 seconds |
| Fresh Product Patterns consumer | passed in 12.3 seconds |
| Accessibility matrix | 55 passed, 30 intentional skips, 10 required contracts |
| Landing initial JavaScript | 63.1 kB gzip |
| Heaviest component route | 417.4 kB JavaScript gzip |
| Local production-preview LCP | 432 ms landing, 452 ms documentation |
| Local interaction timing | 6 ms Shared Detail, 9 ms Dialog |

## Route ownership

- Landing: 1
- Documentation: 13
- Foundations: 5
- Components: 45
- Patterns: 5

## Visual ownership

- Public routes: 138 light/dark snapshots per platform.
- Component Product specimens: 90 snapshots per platform.
- Component State contracts: 90 snapshots per platform.
- Analytics recipes: 8 snapshots per platform.
- Product Patterns: 6 snapshots per platform.
- Snapshot platforms: darwin, linux.

## Claim boundary

The repository verifies the local contracts above. External adoption, unavailable operating systems and physical devices, independent accessibility review, and npm publication remain separate evidence gates.
