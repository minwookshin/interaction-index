...
const fixture = await mkdtemp(join(tmpdir(), "whatiuse-analytics-consumer-"));
const executable = process.platform === "win32"
  ? resolve(root, "node_modules/.bin/shadcn.cmd")
  : resolve(root, "node_modules/.bin/shadcn");
const evidencePath = resolve(root, "release/whatiuse-analytics-install.json");
...
async function startRegistryServer() {
  ...
  if (!address || typeof address === "string") throw new Error("[whatiuse-analytics-consumer] registry server did not bind a port");
...
try {
  ...
  if (!template.includes("{name}")) throw new Error("[whatiuse-analytics-consumer] registry template must contain {name}");

  await mkdir(resolve(fixture, "src"), { recursive: true });
  await writeFile(resolve(fixture, "src/index.css"), "/* whatiuse registry components load their own scoped CSS. */\n");
  await writeFile(resolve(fixture, "package.json"), `${JSON.stringify({
    name: "whatiuse-analytics-consumer",
    private: true,
    version: "0.0.0",
    type: "module",
    dependencies: {
  ...
  await writeFile(resolve(fixture, "index.html"), '<!doctype html><html lang="en"><head><meta charset="UTF-8" /><meta name="viewport" content="width=device-width, initial-scale=1.0" /><title>whatiuse Analytics consumer</title></head><body><div id="root"></div><script type="module" src="/src/main.tsx"></script></body></html>\n');
  ...
  const install = await exec(executable, ["add", "@whatiuse/whatiuse-analytics", "-y", "-c", fixture], {
    cwd: root,
    env: { ...process.env, CI: "1" },
    maxBuffer: 32 * 1024 * 1024,
    timeout: 180_000,
  ...
  const verifiedFiles = [
    ...
    "src/components/ui/facet-filter.tsx",
    "src/components/ui/saved-view-menu.tsx",
    "src/components/ui/data-export-menu.tsx",
    "src/components/ui/data-result-summary.tsx",
    ...
    "src/lib/whatiuse-analytics-contract.ts",
    "src/styles/patterns/analytics-recipes.css",
  ];
  for (const path of verifiedFiles) {
    await access(resolve(fixture, path)).catch(() => {
      throw new Error(`[whatiuse-analytics-consumer] registry install omitted ${path}`);
    });
  }
  ...
  for (const proof of ["Recurring revenue", "SaaS Overview", "Usage & Adoption", "Conversion & Retention"]) {
    if (!builtJavaScript.includes(proof)) throw new Error(`[whatiuse-analytics-consumer] production bundle omitted ${proof}`);
  }

  const elapsedMs = Math.round(performance.now() - startedAt);
  if (process.env.WHATIUSE_ANALYTICS_EVIDENCE === "1") {
    await writeFile(evidencePath, `${JSON.stringify({
      schemaVersion: 1,
      generatedBy: "scripts/verify-whatiuse-analytics-consumer.mjs",
      generatedAt: new Date().toISOString(),
      version,
      status: "passed",
      fixture: "Fresh React + TypeScript + Vite application using the source registry",
      item: "whatiuse-analytics",
      recipes: ["SaaS Overview", "Usage & Adoption", "Conversion & Retention"],
      components: ["Metric", "Sparkline", "Chart", "DonutChart", "Heatmap", "Comparison", "Breakdown", "Goal", "Funnel", "Cohort", "Timeline", "DataToolbar", "DateRangeFilter", "FacetFilter", "SavedViewMenu", "DataExportMenu", "DataResultSummary", "PropertyList"],
      verifiedFiles,
  ...
  console.log(`[whatiuse-analytics-consumer] ${version} installed, type-checked, and built whatiuse Analytics in ${elapsedMs} ms`);
} finally {
