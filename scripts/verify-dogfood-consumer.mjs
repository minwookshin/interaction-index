...
const work = await mkdtemp(join(tmpdir(), "interaction-index-dogfood-"));
...
try {
  ...
  manifest.dependencies["interaction-index"] = `file:${tarball}`;
  await writeFile(manifestPath, `${JSON.stringify(manifest, null, 2)}\n`);

  const appSource = await readFile(resolve(fixture, "src/App.tsx"), "utf8");
  if (!appSource.includes('from "interaction-index"')) {
  ...
  for (const selector of [".ix-action-list", ".ix-dialog", ".ix-shared-detail", ".ix-undo-bar", ".ix-toast"]) {
