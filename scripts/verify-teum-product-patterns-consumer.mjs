...
const fixture = await mkdtemp(join(tmpdir(), "teum-product-patterns-consumer-"));
const executable = process.platform === "win32" ? resolve(root, "node_modules/.bin/shadcn.cmd") : resolve(root, "node_modules/.bin/shadcn");
const evidencePath = resolve(root, "release/teum-product-patterns-install.json");
let server;
...
async function startRegistryServer() {
  ...
  if (!address || typeof address === "string") throw new Error("[teum-product-patterns-consumer] registry server did not bind a port");
  return `http://127.0.0.1:${address.port}/{name}.json`;
}

try {
  const template = process.env.TEUM_REGISTRY_TEMPLATE ?? await startRegistryServer();
  if (!template.includes("{name}")) throw new Error("[teum-product-patterns-consumer] registry template must contain {name}");
  ...
  await writeFile(resolve(fixture, "package.json"), `${JSON.stringify({
    name: "teum-product-patterns-consumer", private: true, version: "0.0.0", type: "module",
  ...
  await writeFile(resolve(fixture, "index.html"), '<!doctype html><html lang="en"><head><meta charset="UTF-8" /><meta name="viewport" content="width=device-width,initial-scale=1" /><title>Teum Product Patterns consumer</title></head><body><div id="root"></div><script type="module" src="/src/main.tsx"></script></body></html>\n');
  ...
  await exec(executable, ["registry", "add", `@teum-pinned=${template}`, "-c", fixture], { cwd: root, env: { ...process.env, CI: "1" }, maxBuffer: 32 * 1024 * 1024, timeout: 120_000 });
  const install = await exec(executable, ["add", "@teum-pinned/teum-product-patterns", "-y", "-c", fixture], { cwd: root, env: { ...process.env, CI: "1" }, maxBuffer: 32 * 1024 * 1024, timeout: 180_000 });
  ...
  const verifiedFiles = [
    ...
    "src/lib/teum-product-patterns-contract.ts",
  ...
  for (const path of verifiedFiles) await access(resolve(fixture, path)).catch(() => { throw new Error(`[teum-product-patterns-consumer] registry install omitted ${path}`); });
  ...
  for (const proof of ["Customer Workspace", "Billing & Usage", "Members & Permissions"]) if (!builtJavaScript.includes(proof)) throw new Error(`[teum-product-patterns-consumer] production bundle omitted ${proof}`);

  const elapsedMs = Math.round(performance.now() - startedAt);
  if (process.env.TEUM_PRODUCT_PATTERNS_EVIDENCE === "1") await writeFile(evidencePath, `${JSON.stringify({
    schemaVersion: 1, generatedBy: "scripts/verify-teum-product-patterns-consumer.mjs", generatedAt: new Date().toISOString(), version, status: "passed",
    fixture: "Fresh React + TypeScript + Vite application using the source registry", item: "teum-product-patterns",
  ...
  console.log(`[teum-product-patterns-consumer] ${version} installed, type-checked, and built three Product Patterns in ${elapsedMs} ms`);
} finally {
