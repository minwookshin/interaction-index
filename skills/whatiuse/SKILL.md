---
name: whatiuse
description: Choose, install, compose, and verify whatiuse React and TypeScript components, product primitives, analytics, and B2B patterns. Use when building or revising a product interface with whatiuse; deciding between adjacent controls such as Select, Combobox, Context Switcher, Menu, Action List, Dialog, Toast, or Undo Stack; selecting a whatiuse Data, Analytics, or Product Pattern recipe; or auditing generated whatiuse code for undocumented APIs, incorrect state ownership, accessibility gaps, or forbidden composition choices.
---

# whatiuse

Build from whatiuse's public contracts. Prefer a verified product recipe over assembling primitives from scratch.
...
The output reports the configured registry, installed whatiuse source, the strongest authored recipe match, its public module and export, and the pinned install command. If there is no explicit match, do not guess a recipe.

## Workflow

1. Inspect `components.json`, the existing semantic tokens, and installed whatiuse source. Preserve the project's React, TypeScript, CSS, aliases, and framework conventions.
...
- whatiuse copies source into the consumer. Inspect and own the installed files.
...
- Never invent `@whatiuse/*` item names. Search `references/catalog.json` or the configured registry.
