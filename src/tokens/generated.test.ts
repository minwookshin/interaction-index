...
describe("generated design tokens", () => {
  ...
  it("keeps light and dark values on the same semantic contract", () => {
    ...
    expect(tokenByPath["theme.foreground.fg-default"].values.light).toBe("var(--whatiuse-gray-900)");
    expect(tokenByPath["theme.foreground.fg-default"].values.dark).toBe("var(--whatiuse-gray-900)");
  });

  it("exposes type-safe variable references to product code", () => {
    expect(tokenVar("foundation.layout.control-height-md")).toBe("var(--whatiuse-control-height-md)");
    expect(tokenVar("theme.elevation.shadow-flyout")).toBe("var(--whatiuse-shadow-flyout)");
  });
});
