...
export function ComponentStatePreview({ id, state, index }: { id: string; state: string; index: number }) {
  ...
  if (id === "field") return <div className="state-field-preview"><Field invalid={error} disabled={disabled}><FieldLabel>Project name{has(normalized, "required") ? " *" : ""}</FieldLabel><FieldControl className={has(normalized, "focus") ? "state-forced-focus" : undefined} defaultValue={has(normalized, "read") ? "Interaction Index" : undefined} placeholder="Interaction Index" readOnly={has(normalized, "read")} required={has(normalized, "required")} />{has(normalized, "description") && <FieldDescription>Visible to everyone in the workspace.</FieldDescription>}{error && <FieldError>Use a unique project name.</FieldError>}</Field></div>;
