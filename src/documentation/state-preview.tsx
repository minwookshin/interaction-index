...
function InlineContextSwitcherSurface({ state }: { state: string }) {
  const highlighted = has(state, "keyboard highlighted");
  return (
    <div className="state-inline-surface state-inline-surface--people" aria-hidden="true">
      <span><strong>Web</strong><small>Browser product interfaces</small></span>
      <span data-highlighted={highlighted || undefined} data-disabled={has(state, "disabled item") || undefined}>
        <strong>Native</strong><small>Mobile and desktop applications</small>
      </span>
    </div>
  );
}
...
function PopoverState({ state }: { state: string }) {
  ...
  return (
    <div className="state-overlay-stack" data-composition="compound" data-side={has(state, "collision") ? "bottom" : "top"}>
...
function MenuState({ state }: { state: string }) {
  const open = !has(state, "closed");
  return (
    <div className="state-overlay-stack" data-composition="compound">
...
function SheetState({ state }: { state: string }) {
  if (has(state, "closed", "escape dismissed")) {
    return <Button className={has(state, "escape") ? "state-forced-focus" : undefined} size="small">Open properties</Button>;
  }
  return (
    <div className="state-sheet-card" data-phase={has(state, "entering") ? "entering" : has(state, "exiting") ? "exiting" : undefined}>
      <strong>Issue properties</strong>
      <span>{has(state, "long") ? "Title, status, priority, assignee, project, cycle, and labels remain scrollable." : "Update title, status, and priority."}</span>
      <Button className={has(state, "focus") ? "state-forced-focus" : undefined} size="small">Save</Button>
    </div>
  );
}

function ContextMenuState({ state }: { state: string }) {
  const open = !has(state, "closed");
  return (
    <div className="state-context-menu">
      <div className={has(state, "keyboard") ? "context-menu-demo-card state-forced-focus" : "context-menu-demo-card"}>
        <Folder aria-hidden="true" /><span><strong>Motion contract</strong><small>INT-184</small></span>
      </div>
      {open && <div className="state-menu-card" role="presentation"><span data-highlighted={has(state, "highlighted") || undefined}><Copy />Duplicate</span><span data-disabled={has(state, "disabled") || undefined} data-danger={has(state, "destructive") || undefined}>{has(state, "destructive") ? <Trash /> : <Archive />}{has(state, "checked") ? "Show contracts" : has(state, "destructive") ? "Delete" : "Archive"}{has(state, "checked") && <Check />}</span></div>}
    </div>
  );
}
  ...
  if (has(state, "editing", "dirty", "saving", "error")) {
    return (
      <div className="state-inline-edit" data-status={has(state, "error") ? "error" : has(state, "saving") ? "saving" : "editing"}>
        <div><input aria-label="Project title" defaultValue={has(state, "dirty") ? "Interaction system" : "whatiuse"} disabled={has(state, "saving")} />{has(state, "saving") && <Spinner size="small" />}</div>
        {has(state, "error") && <small>Use a unique name.</small>}
      </div>
    );
  }
  return <InlineEdit className={has(state, "focus") ? "state-forced-focus" : undefined} value="whatiuse" onSave={() => undefined} disabled={has(state, "disabled")} />;
}
...
export function ComponentStatePreview({ id, state, index }: { id: string; state: string; index: number }) {
  ...
  if (id === "field") return <div className="state-field-preview"><Field invalid={error} disabled={disabled}><FieldLabel>Project name{has(normalized, "required") ? " *" : ""}</FieldLabel><FieldControl className={has(normalized, "focus") ? "state-forced-focus" : undefined} defaultValue={has(normalized, "read") ? "whatiuse" : undefined} placeholder="whatiuse" readOnly={has(normalized, "read")} required={has(normalized, "required")} />{has(normalized, "description") && <FieldDescription>Visible to everyone in the workspace.</FieldDescription>}{error && <FieldError>Use a unique project name.</FieldError>}</Field></div>;
  if (id === "input-group") return <div className="state-input-group-preview"><InputGroup invalid={error} disabled={disabled}>{has(normalized, "leading") && <InputGroupAddon>https://</InputGroupAddon>}<InputGroupInput className={has(normalized, "focus") ? "state-forced-focus" : undefined} aria-label={state} defaultValue="index.design" />{has(normalized, "keyboard") && <InputGroupAddon side="end"><Kbd>⌘K</Kbd></InputGroupAddon>}{has(normalized, "trailing") && <InputGroupButton aria-label="Copy value"><Copy /></InputGroupButton>}</InputGroup></div>;
  if (id === "kbd") return has(normalized, "single") ? <Kbd>K</Kbd> : has(normalized, "sequence") ? <KbdGroup><Kbd>G</Kbd><span>then</span><Kbd>I</Kbd></KbdGroup> : has(normalized, "button") ? <Button size="small" variant="secondary">Commands <Kbd>⌘K</Kbd></Button> : has(normalized, "input") ? <InputGroup><InputGroupInput aria-label="Search components" placeholder="Search…" /><InputGroupAddon side="end"><Kbd>⌘K</Kbd></InputGroupAddon></InputGroup> : <KbdGroup><Kbd>⌘</Kbd><Kbd>K</Kbd></KbdGroup>;
  if (id === "button-group") return <ButtonGroup aria-label={state} attached={has(normalized, "attached")} orientation={has(normalized, "vertical") ? "vertical" : "horizontal"}><Button size="small" variant={has(normalized, "mixed") ? "primary" : "secondary"} leadingIcon={has(normalized, "icon") ? <Plus /> : undefined}>Preview</Button><Button size="small" variant="secondary" disabled={disabled}>Open</Button></ButtonGroup>;
  if (id === "toolbar") return <Toolbar aria-label={state} orientation={has(normalized, "vertical") ? "vertical" : "horizontal"}><ToolbarButton className={has(normalized, "focus") ? "state-forced-focus" : undefined} aria-label="Bold"><strong>B</strong></ToolbarButton><ToolbarButton aria-label="Add" disabled={disabled}><Plus /></ToolbarButton><ToolbarSeparator /><ToolbarButton aria-label="More options" data-popup-open={has(normalized, "popup") || undefined}><Command /></ToolbarButton>{!has(normalized, "vertical") && <ToolbarInput aria-label="Find" placeholder="Find…" />}</Toolbar>;
  if (id === "text-field") return <TextField aria-label={state} placeholder={has(normalized, "empty") ? "Project name" : undefined} defaultValue={has(normalized, "filled", "read") ? "whatiuse" : undefined} error={error ? "Use a unique name." : undefined} disabled={disabled} readOnly={has(normalized, "read")} leading={has(normalized, "leading") ? <MagnifyingGlass /> : undefined} trailing={has(normalized, "trailing") ? <kbd>⌘K</kbd> : undefined} />;
  ...
  if (id === "context-switcher") return <div className="state-control-stack"><ContextSwitcher className={has(normalized, "focus") ? "state-forced-focus" : undefined} aria-label={state} options={contextOptions} defaultValue="web" disabled={disabled} />{has(normalized, "open", "keyboard highlighted", "disabled item") && <InlineContextSwitcherSurface state={normalized} />}</div>;
  ...
  if (id === "context-menu") return <ContextMenuState state={normalized} />;
  if (id === "dialog") return <DialogState state={normalized} />;
  if (id === "sheet") return <SheetState state={normalized} />;
