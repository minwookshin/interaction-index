  ...
  if (has(state, "editing", "dirty", "saving", "error")) {
    return (
      <div className="state-inline-edit" data-status={has(state, "error") ? "error" : has(state, "saving") ? "saving" : "editing"}>
        <div><input aria-label="Project title" defaultValue={has(state, "dirty") ? "Interaction system" : "Teum"} disabled={has(state, "saving")} />{has(state, "saving") && <Spinner size="small" />}</div>
        {has(state, "error") && <small>Use a unique name.</small>}
      </div>
    );
  }
  return <InlineEdit className={has(state, "focus") ? "state-forced-focus" : undefined} value={has(state, "cancelled") ? "Teum" : "Teum"} onSave={() => undefined} disabled={has(state, "disabled")} />;
}
...
export function ComponentStatePreview({ id, state, index }: { id: string; state: string; index: number }) {
  ...
  if (id === "field") return <div className="state-field-preview"><Field invalid={error} disabled={disabled}><FieldLabel>Project name{has(normalized, "required") ? " *" : ""}</FieldLabel><FieldControl className={has(normalized, "focus") ? "state-forced-focus" : undefined} defaultValue={has(normalized, "read") ? "Teum" : undefined} placeholder="Teum" readOnly={has(normalized, "read")} required={has(normalized, "required")} />{has(normalized, "description") && <FieldDescription>Visible to everyone in the workspace.</FieldDescription>}{error && <FieldError>Use a unique project name.</FieldError>}</Field></div>;
