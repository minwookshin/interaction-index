...
Teum `0.1.0-rc.22` is an MIT-licensed, unpublished npm release candidate. The table below records the environment verified on 2026-08-17; it is evidence, not a broad support promise.
...
- 45 frozen public components, 345 compiler-extracted exports, and 145 semantic tokens, plus 15 additive product components across Teum Data and Teum Analytics.
- 203 source tests across component, documentation, keyboard, state, accessibility, data, analytics, Product Pattern, and agent-contract behavior.
- 102 light/dark Storybook contract runs: 90 Core Product/State checks, six Analytics recipe checks, and six Product Pattern checks.
- 330 visual baselines: 138 full-viewport public-route screenshots, 180 isolated Core Product/State screenshots, six Analytics recipe screenshots, and six Product Pattern screenshots.
...
- shadcn-compatible generation for every Core component, 15 product primitives, nine product recipes, shared contracts, the optional Tailwind CSS v4 bridge, and the complete-system item: 72 declared registry items.
- Deterministic SHA-256 metadata for registry JSON, copied files, compiler-extracted APIs, and semantic tokens.
- A content-locked versioned registry at `/r/v/0.1.0-rc.22/` that pins every internal dependency to the same `@teum-pinned` scope, rejects same-version rewrites, and preserves every earlier artifact.
- A fresh React + TypeScript + Vite consumer executes the documented `shadcn registry add` command, installs Button through `@teum-pinned`, type-checks, and completes a production build. The slowest current Vite run passed in 16.4 seconds locally; that duration is observational, not a support guarantee.
...
- The private package entry retains a React `use client` boundary, matches all 197 runtime exports derived from the compiler-owned public surface, renders a representative tree with Node SSR, and hydrates that markup with zero recoverable mismatch errors in a fresh ESM consumer.
...
- The five-project Playwright matrix covers Chromium, Firefox, WebKit, mobile Chromium, and mobile WebKit. The final `0.1.0-rc.22` run reported 295 applicable checks passed and 350 intentional project-ownership skips, with zero failures or flaky results.
