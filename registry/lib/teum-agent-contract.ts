...
export const teumAgentRecipeContracts = [
  {
    id: "issues-workspace",
  ...
  },
  {
    ...
    title: "Product Usage",
    domain: "analytics",
    intent: "Connect active usage, feature adoption, and release events without coupling transient inspection.",
    signals: ["product usage", "feature adoption", "active usage", "release event", "usage trend", "feature breakdown"],
    registryItem: "teum-analytics",
    modulePath: "components/patterns/analytics-recipes",
    exportName: "ProductUsageRecipe",
    components: ["Metric", "Chart", "Breakdown", "Timeline"],
    state: ["local active index", "visible series", "selected release"],
    rules: ["Keep transient pointer state local to each chart.", "Keep date order stable when series are filtered.", "Map each explicit release selection to one stable annotation."],
    forbidden: ["Do not mirror hover tooltips across adjacent charts.", "Do not remove textual values when a series is hidden.", "Do not animate high-frequency inspection."],
  },
  {
  ...
  },
  {
    ...
    registryItem: "teum-product-patterns",
...
] as const satisfies readonly TeumAgentRecipeContract[];

export const teumAgentSelectionRules = [
...
] as const satisfies readonly TeumAgentSelectionRule[];

export const teumAgentForbiddenRules = [
...
export const teumAgentSystemContract = {
  schemaVersion: 1,
  product: "Teum",
...
export function selectTeumRecipe(task: string): TeumAgentPlan | null {
  const normalizedTask = normalize(task);
  const candidates = teumAgentRecipeContracts.map((recipe) => {
